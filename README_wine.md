...
The first split is on **alcohol**, at roughly 10% by volume — wines below that are markedly more likely to be rated poorly. Running scikit-learn's tree with the same criterion and depth gives 70.6% and roots on the same variable at 9.975, which is a reasonable check that the implementation does what it claims.
